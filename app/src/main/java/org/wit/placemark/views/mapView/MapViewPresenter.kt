...
class MapViewPresenter(view: BaseView): BasePresenter(view) {
    ...
    fun doMarkerClick(marker: Marker) {
        ...
        view!!.mapview_title.text = hillfort.title
        view!!.mapview_description.text = hillfort.description
        view!!.mapview_rating.text = "%.2f".format(hillfort.rating)
        ...
        if(hillfort.location["lat"]!! <= 90) {
            ...
            view!!.mapview_location.text = str
        }

        val user = app.users.findUserByHillfort(hillfort)

        if (user != null) {
            view!!.mapview_user.text = user.userName
        }
        ...
        val adapter =
            ImagePagerAdapter(hillfort.images, view!!.mapview_image.context)
        view!!.mapview_image.adapter = adapter
    ...
    fun openSearch() {
        ...
        val titleLabel = popUp.findViewById<TextView>(R.id.search)
        ...
        val searchButton = popUp.findViewById<Button>(R.id.seach_btn)
        ...
        searchButton.setOnClickListener {
            ...
            if(!errors) {
                ...
                window.dismiss()
                view!!.mapview_search.isClickable = true
            }
